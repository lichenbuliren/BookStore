 ...
 * 这里是登录验证码模块
 */
var loginFormModule = angular.module('LoginFormModule',[]);
loginFormModule.controller('LoginController',function($scope,$state){
    $scope.account = {
        email: 'heaven@taohaikeji.com',
        password: '123456'
    };
    ...
    $scope.login = function(){
        if($scope.userInfo.email != $scope.account.email){
            $scope.userInfo.email = '';
            $('input[type="email"]').focus();
            alert('用户名错误，请重新输入！');
        }

        if($scope.userInfo.password != $scope.account.password){
